...
- 当前维护：目录预览会持久化侧栏宽度、展开状态与正文宽度，统一标题及全部工具栏按钮的工具提示和垂直对齐，凸显当前文档并尽可能将其置于侧栏高度中间。
...
- [x] 顶栏显示服务端文档标题并以悬浮提示呈现路径，主题按钮图标与当前主题一致。
- [x] 桌面侧栏支持拖动调整宽度，正文支持标准、宽、全屏三种预设，树节点以悬浮提示显示完整名称。

### 测试与验收

- [x] Vitest 覆盖默认选择、深链接恢复、前进/后退、树展开定位、刷新与删除回退，以及宽度预设、侧栏拖动、标题路径和主题图标。
