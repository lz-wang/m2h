...
//
// This is deliberately minimal: the browser's native scroll restoration was
// measured NOT to fire for this app's shape (a client-rendered body whose
// content arrives after the first layout), so the tab remembers the offset
// itself — one rAF-throttled write per scroll frame, one restore when the
// document is ready. Late reflows are left to CSS scroll anchoring.
//
