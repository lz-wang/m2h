...
目录模式打开内嵌的 React 界面，桌面端由文件树侧栏和文档区组成；移动端可通过“切换文件导航”按钮打开侧栏。当前文件所在目录会自动展开并滚动定位，其余目录默认折叠，并以克制的高亮标识当前文档；悬浮文档项可同时查看文件名与文档标题。侧栏支持 `⌘/Ctrl+B` 切换，标题栏中的文档标题和所有功能按钮严格对齐，并使用一致的悬浮提示展示路径或功能。拖动后的侧栏宽度、侧栏展开状态与正文宽度预设会保存在浏览器本地，刷新后继续沿用。
