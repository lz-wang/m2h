...
在 VPS 等远程机器上用 `--host 0.0.0.0 --no-open` 对外提供服务时，WebUI 不再暴露服务器本地绝对路径：根路径悬停提示与“复制文档/文件夹本地路径”菜单项随之隐藏。
...
| 特性          | 简单示例                           |
| ----------- | ------------------------------ |
| 语法高亮        | 围栏代码块根据语言标识进行高亮；Web 预览额外提供复制按钮与行号（横向滚动时行号与复制按钮位置保持固定，复制只复制源码），超过 25 行的代码块默认折叠，点击可展开完整内容 |
...
数学公式、Mermaid 图表与表格排序的运行时在 Web 预览中内嵌于二进制、离线可用；HTML 导出按需从 jsDelivr CDN 加载固定版本的同一组运行时。
