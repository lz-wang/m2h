...
正文中的普通图片（包括图片链接与原始 HTML 中的 `<img>`）右上角提供大图查看入口，点击进入全屏 Lightbox：底部半透明工具栏支持在当前文档的图片间上一张/下一张切换（按文档顺序编号，首尾不循环）、缩放（放大后可拖动查看）、±90° 旋转与位置计数，键盘 `←`/`→` 切换图片；点击图片本体仍保持原有行为（图片链接继续跟随链接），关闭按钮、`Esc` 与点击遮罩空白处均可退出，关闭后正文阅读位置与地址栏锚点保持不变。
