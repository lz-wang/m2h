...
`m2h` 是一个轻量、零配置的 Markdown Web 文档浏览与服务工具。直接指定 Markdown 文件或目录即可启动内嵌 WebUI，支持 GFM、数学公式、Mermaid 与文件树导航；同时提供简单的单文件 HTML 导出功能。
...
# 打开目录；提供文件树、搜索、分享、主题、正文宽度、文档目录与正文右下角的回到顶部/前往底部按钮
...
`m2h` 不监视本地文件系统。每次打开文档都会从磁盘读取最新内容；刷新页面会重新扫描目录并读取当前文档，因此内容和目录变化无需重启服务即可生效。修改内容后可再次点击该文档或刷新页面查看更新；新增、删除或重命名文件后刷新页面即可更新文件树。当前浏览的 Markdown 已被删除或路径不存在时，页面保留原 URL 并显示“您浏览的文档不存在或已被删除”。
...
侧边栏与文档目录的滚动条在滚动时显示、停止约 0.7 秒后淡出；右下角的回到顶部/前往底部按钮位于正文区域右下角，开启文档目录时自动左移避开目录栏，窄屏（<1200px）目录栏隐藏后回到视口右缘。
