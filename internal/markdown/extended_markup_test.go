...
func TestKeys(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "combo splits into kbd",
			source:   "++ctrl+alt+del++",
			want:     `<span class="keys">`,
			kbdCount: 3,
		},
		{
			name:   "single alias normalized",
			source: "++ctrl++",
			want:   `<kbd class="key-ctrl">Ctrl</kbd>`,
		},
		{
			name:   "unknown key kept literal",
			source: "++foobar++",
			want:   `<kbd>foobar</kbd>`,
		},
		{
			name:       "C++ stays literal",
			source:     "C++ 语言",
			want:       "C++",
			wantNoKeys: true,
		},
		{
			name:       "plus in prose stays literal",
			source:     "a + b",
			wantNoKeys: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.kbdCount > 0 && strings.Count(body, "<kbd") != tt.kbdCount {
				t.Errorf("got %d <kbd>, want %d:\n%s", strings.Count(body, "<kbd"), tt.kbdCount, body)
			}
			if tt.wantNoKeys && strings.Contains(body, `<span class="keys">`) {
