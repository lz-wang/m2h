...
func TestNewPreviewScopeSingleFileIgnoresDiscovery(t *testing.T) {
	...
	scope := newPreviewScope(input, files.DiscoverOptions{Depth: 5, Pattern: "**/*.md"})
...
func TestNewPreviewScopeDirectoryPreservesDiscovery(t *testing.T) {
	...
	scope := newPreviewScope(input, options)
	if scope.isSingleFile() {
...
func TestSingleFileScopeDiscoversOnlyOneDocument(t *testing.T) {
	...
	scope := previewScope{root: root, file: "guide.md"}
...
func TestDirectoryScopeDiscoverRespectsDepth(t *testing.T) {
	...
	shallow := previewScope{root: root, discovery: files.DiscoverOptions{Depth: 0}}
	...
	deep := previewScope{root: root, discovery: files.DiscoverOptions{Depth: 2}}
...
func TestAllowsDocumentSingleFile(t *testing.T) {
	...
	scope := previewScope{root: t.TempDir(), file: "guide.md"}
...
func TestAllowsDocumentDirectory(t *testing.T) {
	...
	scope := previewScope{root: t.TempDir(), discovery: files.DiscoverOptions{Depth: 1, Pattern: "**/*.md"}}
	if !scope.allowsDocument("guide.md") {
