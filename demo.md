...
实时示例：复制用 ++ctrl+c++，切换窗口用 ++alt+tab++，三键组合 ++ctrl+shift+esc++ 打开任务管理器。全部按键与别名的完整渲染效果见[键盘按键全览](docs/demos/pymarkdown/keyboards.md)。
...
全部标记与行为边界的完整展示见 [Critic 协作标记全览](docs/demos/pymarkdown/critic.md)。
